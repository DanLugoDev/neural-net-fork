...
export default memo(NeuralNetworkInteractive)

const  NeuronConnections = memo(({ selectedNeuron, animating, instant }) => {
  let connections = [];
...
const WinningOutputNeuronBox = memo(({ neurons, animating, instant }) => {
...
const PiCreature = memo(({ animating, instant }) => {
  return (
    <>
      <g
...
const VerticalEllipsis = memo(({ cx = 0, cy = 0 }) => {
  return (
    <g>
...
const WeightGrid= memo(({ x, y, width, height, weights, inputNeurons })=> {
